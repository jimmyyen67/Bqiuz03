...
//取出設定為顯示並排序過的資料
$rows=$po->all(['showing'=>1]," order by `rank`");
      ...
      <!--海報區-->
        ...
        <?php
          foreach($rows as $k=>$row){
            echo "<div class='po' data-ani='".$row['animation']."'>";
            echo "<img src='image/".$row['file']."'>";
...
<style>
  ...
  <!---院線片-->  
    <div class="half">
      <h1>院線片清單</h1>
      <div class="rb tab" style="width:95%;min-height:365px;position:relative;">
      <?php
        ...
        $total=$db->count(['showing'=>1]," && ondate >= '$ondate' && ondate <='$today'");
        $div=4;
        $pages=ceil($total/$div);
        $now=(!empty($_GET['p']))?$_GET['p']:1;
        $start=($now-1)*$div;
        $rows=$db->all(['showing'=>1]," && ondate >= '$ondate' && ondate <='$today' order by rank limit $start,$div");
      ...
      <div class="mb">
          <table class="m-auto">
            <tr>
              <td rowspan="3"><a href="?do=intro&id=<?=$row['id'];?>"><img src="image/<?=$row['poster'];?>" style="width:80px;height:100px;"></a></td>
              <td class="ct"><?=$row['name'];?></td>
            </tr>
            <tr>
              <td class="ct"><img src="icon/<?=$row['level'];?>.png" style="width:15px"><?=$level[$row['level']];?></td>
            </tr>
            <tr>
              <td class="ct"><?=$row['ondate'];?></td>
            </tr>
          </table>
          <div class="ct">
